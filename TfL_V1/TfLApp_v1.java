...
public class TfLApp_v1 {
    ...
    static Graph buildNetwork() {
        ...
        // Monument merged into Bank (same station complex; Circle/District edges below
        // use "Bank")
    ...
    private static void addCircle(Graph g) {
        // Inner = anti-clockwise (Edgware Road → Paddington → south loop → City →
        // back).
        // Outer = clockwise (Hammersmith → H&C stem → Baker Street → City → south loop
        // → back).
    ...
    private static void addHammersmithAndCity(Graph g) {
        ...
        g.addEdge("Wood Lane", "Shepherd's Bush Market", "H & C", "Westbound", 1.50); // blank in source; using
                                                                                      // eastbound value
    ...
    private static void addNorthern(Graph g) {
        ...
        // because both carry the same line+direction label "Northern
    ...
    private static void showMenuLoop(Graph g, Scanner sc) {
        while (true) {
            ...
            System.out.println("10. Benchmark");
    ...
    private static void modifyDelay(Graph g, Scanner sc, boolean adding) {
        ...
        boolean both = readYesNo(sc, "Apply to reverse direction (" + g.getStation(to).name
                + " -> " + g.getStation(from).name + ") too? (y/N): ");
        ...
        if (both) {
            String reverseDir = reverseDirection(direction);
            changed += adding
                    ? g.addDelay(to, from, line, reverseDir, minutes)
                    : g.removeDelay(to, from, line, reverseDir, minutes);
        }

        if (changed == 0) {
            System.out.println(adding
                    ? "No matching edges found."
                    : "No active delays to remove on the matching edges.");
        } else {
            System.out.println("Delay " + (adding ? "added" : "removed")
                    + " on " + changed + " track section(s).");
        }
    }
    ...
    private static void modifyTrack(Graph g, Scanner sc, boolean closing) {
        ...
        if (both) {
            String reverseDir = reverseDirection(direction);
            changed += closing
                    ? g.closeTrack(to, from, line, reverseDir)
                    : g.openTrack(to, from, line, reverseDir);
        }

        if (changed == 0) {
            System.out.println(closing
                    ? "All matching tracks were already closed."
                    : "All matching tracks were already open.");
        } else {
            System.out.println("Track " + (closing ? "closed" : "opened")
                    + " on " + changed + " section(s).");
        }
    }
    ...
    private static void showBenchmark(Graph g, Scanner sc) {
        System.out.print("Warm-up runs: ");
        int warmup = readInt(sc);
        System.out.print("Benchmark runs: ");
        int runs = readInt(sc);
        if (warmup < 0 || runs < 1) {
            System.out.println("Warm-up runs must be >= 0 and benchmark runs must be >= 1.");
            return;
        }
        runBenchmark(g, warmup, runs);
    }
    ...
    /**
     * Maps a line direction to its opposite, used when applying engineer ops
     * to both physical directions of a track section. Returns null unchanged
     * (null means "match any direction").
     */
    ...
    // -----------------------------------------------------------------------
    // Timed benchmark running Dijkstra on all station pairs, to get an average runtime per route.
    // -----------------------------------------------------------------------

    static long benchmarkRoutes(Graph g) {
    ...
    static void runBenchmark(Graph g, int warmupRuns, int runs) {
        PrintStream original = System.out;
        System.setOut(new PrintStream(OutputStream.nullOutputStream()));
        try {
            original.print("Running benchmark");
            ...
            for (int r = 0; r < runs; r++) {
                grandTotal += benchmarkRoutes(g);
                original.print(".");
            }
            original.println(" done");

            System.setOut(original);
            System.out.println("Grand average: " + (grandTotal / runs) / 1_000_000.0 + " ms");
        } finally {
            System.setOut(original);
        }
    }
    ...
    static void runSample(Graph g) {
        ...
        // close Jubilee Eastbound: Bond Street -> Green Park
        g.addDelay(
        ...
        // Spec section 2.2(2)(ii) station information.
        System.out.println();
    ...
    public static void main(String[] args) {
        ...
        // If "sample" is passed as a command-line argument, run the sample output from
        // the spec.
