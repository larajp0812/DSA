...
public class TfLApp_v2 {
    ...
    private static void showBenchmark(GraphV2 g, Scanner sc) {
    ...
    // -----------------------------------------------------------------------
    // Timed benchmark running Dijkstra on all station pairs, to get an average
    // runtime per route.
    // -----------------------------------------------------------------------

    static long benchmarkRoutes(GraphV2 g) {
            ...
            for (int from = 0; from < n; from++) {
                for (int to = 0; to < n; to++) {
                    if (from == to)
                        continue;
                    long start = System.nanoTime();
                    boolean found = DijkstraV2.findFastestRoute(g, from, to);
                    long end = System.nanoTime();
                    if (found) {
                        total += (end - start);
                        count++;
    ...
    static void runBenchmark(GraphV2 g, int warmupRuns, int runs) {
        PrintStream original = System.out;
        System.setOut(new PrintStream(OutputStream.nullOutputStream()));
        try {
            ...
            for (int i = 0; i < warmupRuns; i++) {
                DijkstraV2.findFastestRoute(g, 0, g.getStationCount() - 1);
                original.print(".");
            }
